...
<body>
  <div id="wrapper">
      ...
      <div class="MainContent well Mensaje">
		...
		<h3>Actualizar Dentista <?php echo $Rut ?></h3>
        ...
        if($con){ 
        	...
        	if($res){ 
        		...
        		<form class="form-horizontal well" method="post" action="/Clinica/Administrador/Dentista/edit2.php?Rut=<?php echo $Rut ?>">
				    <fieldset>
		        		...
		        		<?php
          				$consulta="SELECT * FROM especialidad E, Tiene T WHERE E.esp_tipo='DENTISTA' AND E.esp_id=T.esp_id AND T.med_rut='$Rut'"; 
          				$res=pg_query($consulta);
          				$consulta2="SELECT esp_id,esp_nombre FROM especialidad WHERE esp_tipo='DENTISTA' EXCEPT (SELECT e.esp_id,e.esp_nombre FROM medico m, tiene t, especialidad e WHERE m.med_rut=t.med_rut AND t.esp_id=e.esp_id AND m.med_rut='$Rut' AND e.esp_tipo='DENTISTA')"; 
			      ...
			      <?php } ?>
				        <div class="form-actions">
				            ...
				            <button type="reset" class="btn btn-danger" onclick="window.location='/Clinica/Administrador/Dentista/index.php'">Atras</button>
				        </div>
				    </fieldset>
				</form>
      <?php }else{ ?>
